...
CASE WHEN uc.job_title LIKE '%Testing Manager%' THEN 'TM'
...
ELSE 'Other' END AS creator_role,
pt.is_done,
CASE WHEN pt.is_done = 0 THEN 'allocated, not completed'
WHEN pt.is_done = 1 AND pt.is_approved = 0 THEN 'completed, not approved'
END AS is_done_str,
(CASE WHEN pt.foreign_key_type = 'time_reporting' THEN pt.starts_at
ELSE pt.done_at
